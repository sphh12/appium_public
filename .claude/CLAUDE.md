...
큰 작업(Phase 단위, 여러 단계에 걸친 구현 등)을 진행 중일 때, **각 주요 단계 완료 시점**마다 진행 상태를 기록한다.
컨텍스트 소진으로 세션이 끊겨도 다음 세션에서 빠르게 이어받을 수 있도록 하기 위함.

**기록 위치**: `Todo.md`의 `진행 중` 섹션
**기록 형식**: `- **[작업명]**: N/M단계 완료, 다음: [다음 단계 설명]`

예시:
```
- **Phase 2 구현**: 4/7단계 완료, 다음: ArtifactViewer 컴포넌트 구현
- **iOS 환경 구축**: Xcode 설치 완료, 다음: XCUITest 드라이버 설정
```
...
3. **개인정보/민감정보 스캔**: 커밋 전 `git diff`에서 다음 항목이 노출되는지 확인
   - API 키, 토큰, 비밀번호 (실제 값)
   - 이메일, 전화번호, 개인 식별 정보
   - `.env` 파일의 실제 값이 포함되지 않았는지
   - 문제 발견 시 사용자에게 알리고 커밋 중단
4. 현재 바라보는 브랜치에 변경사항을 **GitHub과 GitLab 모두** 푸시 (`GIT_RULES.md` 규칙 준수)
...
**동작:**
1. `change_notes.md` 파일에서 최근 완료 작업 확인
2. `Todo.md` 파일에서 진행 중/다음 단계 작업 확인
3. 간결하게 요약 브리핑:
   - 최근 완료 작업 (1~3줄)
   - 남은 할일 목록 (우선순위순)
   - 다음 추천 작업 제안
