...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --livetest                Use LiveTest APK"
            ...
            echo "  --<file>                  Shorthand for tests/<platform>/<file>.py (e.g., --xml_test or --sample1_test)"
            ...
            echo "  ./shell/run-app.sh --basic_01_test --livetest    # LiveTest APK"
            echo "  ./shell/run-app.sh --sample1_test --test test_Login # STG + specific test"
...
if [[ "$PLATFORM" == "android" && -z "$APP_PATH" ]]; then
    ...
    # ENV_TYPE을 폴더명으로 매핑
    case "$ENV_TYPE" in
        stg)      APK_FOLDER="stage" ;;
        live)     APK_FOLDER="live" ;;
        livetest) APK_FOLDER="livetest" ;;
        *)        APK_FOLDER="$ENV_TYPE" ;;
    esac

    # 폴더 내 .apk 파일 자동 탐색 (이름순 마지막 = 최신)
    APK_DIR="$PROJECT_ROOT/apk/$APK_FOLDER"
    if [[ -d "$APK_DIR" ]]; then
        ...
        echo -e "${RED}[ERROR] APK folder not found: $APK_DIR${NC}"
...
DEFAULT_TARGET="tests/android/sample1_test.py"
if [[ "$PLATFORM" == "ios" ]]; then
