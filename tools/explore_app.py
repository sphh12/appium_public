...
def setup_driver():
    ...
    if USE_LIVE:
        caps["appActivity"] = f"{pkg}.SplashActivity"
    else:
        caps["appActivity"] = f"{pkg}.MainActivity"
...
def get_app_package():
    ...
    # "com.example.remittance.stag:id" → "com.example.remittance.stag"
    return RID.split(":")[0]
...
def _log_interactive_elements(sources, base_name):
    ...
    for source in sources:
        ...
        for elem in root.iter():
            ...
            # 패키지명 접두사 제거 (예: com.example.remittance.online:id/btn_ok → btn_ok)
...
def explore_hamburger(driver, folder):
    ...
    try:
        ...
        # APP_STRUCTURE_stg.md에서 확인된 메뉴 항목
        menu_by_id = [
...
def explore_history(driver, folder):
    ...
    # 스크롤해서 카테고리 항목 찾기
    categories = ["Remittance", "Account", "SamplePay", "Top-up", "Coupon Box"]
...
def explore_card_3rd_depth(driver, folder):
    ...
    top_elements = [
        ...
        ("c2", "btn_wallet_transfer", "Transfer", False),
        ("c3", "btn_wallet_deposit", "Deposit", False),
    ...
    try:
        ...
        # ViewPager 영역 찾기
        vp = driver.find_element(AppiumBy.ID, _id("cardViewPager"))
        vp_rect = vp.rect
        ...
        print("  [warn] c10: 'cardViewPager' 없음 → 스킵")
    except Exception as e:
