...
<android.widget.Button
    index="1"
    package="com.example.remittance.stag"
    class="android.widget.Button"
    text="Login"
    resource-id="com.example.remittance.stag:id/btn_submit"
    checkable="false"
    checked="false"
    clickable="true"
    enabled="true"
...
## 캡처된 화면 분석 (Sample Remit 앱)

### 001.xml - 안드로이드 홈 화면

| 요소          | content-desc                                | 용도    |
| ------------- | ------------------------------------------- | ------- |
| Sample 앱 아이콘 | `Sample Remit` 또는 `Predicted app: Sample Remit` | 앱 실행 |
...
try:
    ...
    element = driver.find_element(
        AppiumBy.ID,
        "com.example.remittance.stag:id/ivChannelTalk"
    )
```
...
class TestXmlScenario:
    PACKAGE_ID = "com.example.remittance.stag:id"
